...
export default function NeuralParticleCanvas({
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < coreCount; i++) {
      ...
      const spawnRadius = 4.5;
      const laneRoll = Math.random();
      const laneCenter = laneRoll < 0.34 ? Math.PI : laneRoll < 0.68 ? 0 : -Math.PI / 2;
    ...
    const animate = () => {
      ...
      if (mode === "converge" || mode === "diverge") {
        ...
        if (mode === "converge") {
          ...
          ringMats.forEach((rm, r) => { rm.opacity = eased * (0.15 + r * 0.05); });
          ...
          ringMats.forEach((rm, r) => { rm.opacity = (0.15 + r * 0.05) * (1 - eased); });
