...
export function PremiumControlTowerBanner() {
  ...
  return (
    <motion.div
      ...
      className="relative overflow-hidden rounded-2xl border border-emerald-500/20 bg-gradient-to-br from-zinc-950 via-zinc-900 to-emerald-950/30"
    >
      {/* Ambient glow */}
      ...
      <div className="relative z-10 p-5 sm:p-6 flex flex-col sm:flex-row items-start sm:items-center gap-4">
        ...
        <Link to={hasAccess ? "/control-tower" : "/pricing"}>
          <Button
            size="sm"
            className={hasAccess
              ? "bg-gradient-to-r from-emerald-600 to-emerald-500 hover:from-emerald-500 hover:to-emerald-400 text-white border-0 gap-1.5 shadow-lg shadow-emerald-500/20"
              : "bg-white/10 hover:bg-white/15 text-white/60 border border-white/10 gap-1.5"
            }
          >
            {hasAccess ? (
              ...
              <>
                <Lock className="w-3.5 h-3.5" /> Upgrade para PRO
              </>
            )}
          </Button>
        </Link>
