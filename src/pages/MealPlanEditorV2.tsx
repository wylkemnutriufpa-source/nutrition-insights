...
export default function MealPlanEditorV2() {
    ...
    if (np?.default_meal_plan_id) {
      ...
      try {
        ...
        if (defaultItems && defaultItems.length > 0) {
          ...
          toast.success("Template padrão carregado com sucesso!", { id: toastId });
        } else {
  ...
  useEffect(() => {
    if (store.hydrated && store.items.length === 0 && !isImmutable) {
      loadDefaultTemplate();
    }
  }, [store.hydrated]);
