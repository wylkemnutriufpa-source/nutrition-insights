          ...
          return (meal.items || []).flatMap((item: any) => [
            {
              ...
              meta_calorias: Math.round(Number(item.macros?.kcal ?? item.kcal) || 0),
              meta_proteinas: Math.round(Number(item.macros?.protein_g ?? item.protein) || 0),
              meta_carboidratos: Math.round(Number(item.macros?.carbs_g ?? item.carbs) || 0),
              meta_gorduras: Math.round(Number(item.macros?.fat_g ?? item.fat) || 0),
              is_primary: true,
              substitution_group_id: item.blockId || item.id,
              day_of_week: day.day_of_week,
              visual_image_url: item.imageUrl || item.image_url || item.image || null,
              meal_image_url: meal.image || meal.image_url || item.imageUrl || null,
            ...
            ...(item.substitutions || []).map((sub: any) => ({
              ...
              description: sub.quantity_display || sub.qty,
              meta_calorias: Math.round(Number(sub.macros?.kcal ?? sub.kcal) || 0),
              meta_proteinas: Math.round(Number(sub.macros?.protein_g ?? sub.protein) || 0),
              meta_carboidratos: Math.round(Number(sub.macros?.carbs_g ?? sub.carbs) || 0),
              meta_gorduras: Math.round(Number(sub.macros?.fat_g ?? sub.fat) || 0),
              is_primary: false,
              substitution_group_id: item.blockId || item.id,
              day_of_week: day.day_of_week,
              visual_image_url: sub.imageUrl || sub.image_url || sub.image || null,
