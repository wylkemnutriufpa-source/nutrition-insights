  ...
  async getPlanById(planId: string): Promise<PatientPlan | null> {
    ...
    if (data.editor_version === 'v3') {
      ...
      return {
        ...
        meta_calorias: Number(snapshot.targets?.kcal || data.total_target_calories || 0),
        meta_proteinas: Number(snapshot.targets?.protein_g || data.total_target_protein || 0),
        meta_carboidratos: Number(snapshot.targets?.carbs_g || data.total_target_carbs || 0),
        meta_gorduras: Number(snapshot.targets?.fat_g || data.total_target_fat || 0),
        meals: mappedMeals,
    ...
    return {
      ...
      patient_name: patientData?.notes || 'Paciente',
      goal: patientData?.status || '',
      meta_calorias: data.total_target_calories || 0,
      meta_proteinas: data.total_target_protein || 0,
      meta_carboidratos: data.total_target_carbs || 0,
      meta_gorduras: data.total_target_fat || 0,
