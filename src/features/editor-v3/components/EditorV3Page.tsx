        ...
        <SharePlanDialog 
          ...
          data={patientData ? {
            ...
            items: store.meals.flatMap(meal => meal.items.flatMap(item => [
              ...
              ...(item.substitutions || []).map(s => ({ id: s.instanceId || s.id, mealType: meal.name, title: s.name, meta_calorias: s.kcal, meta_proteinas: s.protein, meta_carboidratos: s.carbs, meta_gorduras: s.fat, day_of_week: meal.day_of_week || 0, scheduled_time: meal.time, is_primary: false, clinical_mass_g: s.clinical_mass_g }))
            ])),
