  ...
  return useQuery({
    ...
    queryFn: async () => {
      const { data: resolvedProfile, error: resolvedProfileError } = await withTenantFilter(
        supabase
          .from("profiles")
          .select("id, user_id, full_name, avatar_url, phone, fit_intelligence_enabled, patient_state, fit_intelligence_access_mode, fit_intelligence_expires_at, fit_intelligence_first_experience_seen, marmita_mode, current_weight_kg, current_height_cm, activity_level, goal, restrictions, preferences")
