-- ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
-- INTEGRAÇÃO DOS TEMPLATES NO EDITOR V3 - VERSÃO DEFINITIVA
-- ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
-- Inserir TODOS os templates criados hoje
...
SELECT 
  ...
  COALESCE(
    t.description, 
    CASE 
      WHEN (t.dietary_restrictions->>'gluten_free')::boolean = true 
        OR (t.dietary_restrictions->>'lactose_free')::boolean = true 
        OR (t.dietary_restrictions->>'low_fodmap')::boolean = true 
      THEN 'Template com restrições alimentares'
      ELSE 'Template soberano com refeições completas e modulares'
    END
  ) as description,
  CASE t.category
  ...
  CASE 
    WHEN (t.dietary_restrictions->>'gluten_free')::boolean = true 
      OR (t.dietary_restrictions->>'lactose_free')::boolean = true 
      OR (t.dietary_restrictions->>'low_fodmap')::boolean = true 
    THEN 'Restrições Alimentares'
    WHEN t.category = 'emagrecimento' THEN 'Emagrecimento'
    WHEN t.category = 'hipertrofia' THEN 'Hipertrofia'
    WHEN t.category = 'saude' THEN 'Saúde'
    WHEN t.category = 'clinico' THEN 'Clínico'
    ELSE 'Geral'
  END as family,
  COALESCE(
    (
...
WHERE t.created_at >= '2026-05-21'
  ...
  AND SUBSTRING(t.name FROM '\d+') IS NOT NULL
