...
serve(async (req) => {
  ...
  try {
    ...
    const { data: insightRow, error: insightErr } = await serviceClient.from("anamnesis_ai_insights").insert({ user_id: anamnesis.user_id, anamnesis_id, risk_level: insights.risk_level, primary_goal: insights.primary_goal, metabolic_profile: insights.metabolic_profile, main_pains: insights.main_pains, nutrition_focus: insights.nutrition_focus, behavior_focus: insights.behavior_focus, movement_focus: insights.movement_focus, suggested_protocol: insights.suggested_protocol, personalized_tips: insights.personalized_tips, ai_summary: insights.ai_summary, raw_response: auditMetadata }).select().single();
