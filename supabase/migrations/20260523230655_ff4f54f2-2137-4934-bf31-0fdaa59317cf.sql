-- Add patient_id to invitations table
ALTER TABLE public.invitations ADD COLUMN IF NOT EXISTS patient_id UUID REFERENCES auth.users(id);

-- Update complete_invitation RPC to be much more robust
...
BEGIN
  ...
  -- We fetch the patient's existing profile data to ensure we don't overwrite it with nulls
  SELECT full_name, email, phone, whatsapp INTO v_patient_profile
  ...
  SELECT public.create_patient_canonical(
    ...
    _email := COALESCE(v_patient_profile.email, v_invitation.patient_email),
