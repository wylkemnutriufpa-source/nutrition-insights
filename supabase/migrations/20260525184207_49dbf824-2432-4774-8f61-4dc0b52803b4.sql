-- 1. Fix the trigger function that was causing the crash
...
BEGIN
    ...
    IF NEW.status = 'completed' AND (OLD.status IS NULL OR OLD.status <> 'completed') THEN
        -- Atualiza o perfil (Usa onboarding_completed e clinical_assessment_completed que existem)
...
-- 2. Improve handle_new_user with more robust error handling for metadata
...
BEGIN
  ...
  -- 5. Lógica para PACIENTES
  ELSIF v_role = 'patient' THEN
    -- Try to cast UUID safely
    BEGIN
    ...
    -- FIX: Search by email if still null
    ...
    IF v_nutri_id IS NOT NULL THEN
      IF v_tenant_id IS NULL THEN
        SELECT id INTO v_tenant_id FROM public.user_tenants WHERE user_id = v_nutri_id AND role = 'owner' LIMIT 1;
        IF v_tenant_id IS NULL THEN
