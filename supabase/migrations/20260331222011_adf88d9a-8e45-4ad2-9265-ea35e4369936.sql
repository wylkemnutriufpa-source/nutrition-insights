...
BEGIN
  ...
  IF NOT _plan.is_active AND _plan.plan_status <> 'published_to_patient' THEN
    RETURN jsonb_build_object('success', true, 'message', 'Already inactive');
  END IF;

  IF _plan.plan_status = 'published_to_patient' THEN
    UPDATE public.meal_plans
    SET is_active = false,
        plan_status = 'archived',
        updated_at = now()
  ...
  VALUES (
    ...
    'plan_deactivated',
    'Plano desativado',
    'Plano alimentar foi desativado pelo profissional.'
  );

  RETURN jsonb_build_object('success', true, 'archived', _plan.plan_status = 'published_to_patient');
END;
$$;