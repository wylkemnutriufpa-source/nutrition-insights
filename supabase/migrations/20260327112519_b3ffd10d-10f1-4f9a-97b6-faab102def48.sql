...
BEGIN
  ...
  -- Idempotent: if already onboarding_active, just return success
  IF _current_status = 'onboarding_active' THEN
    RETURN jsonb_build_object('success', true, 'message', 'Onboarding já está ativo');
  END IF;

  -- Auto-create consent if missing (professional is authorizing)
