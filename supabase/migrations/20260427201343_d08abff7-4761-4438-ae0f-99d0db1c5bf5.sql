...
BEGIN
  ...
  -- Se não há vínculo nenhum, permitimos o acesso (o dashboard lidará com o estado vazio)
  IF NOT FOUND THEN
    RETURN jsonb_build_object('status','ok','issues', ARRAY['no_nutritionist_link'], 'actions', '[]'::jsonb);
  END IF;

  -- 3. Blindagem de Estados Fluidos
  -- Pacientes com status de jornada definidos são considerados "Ready" para o Dashboard,
  -- pois o próprio Dashboard gerencia as modais de progresso/onboarding.
