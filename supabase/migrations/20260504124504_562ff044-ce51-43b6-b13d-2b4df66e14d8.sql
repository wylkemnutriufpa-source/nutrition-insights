...
BEGIN
  ...
  ELSE
    -- Keep current or default to slides if never started
  ...
  IF v_new_state <> v_current_state OR v_current_state IS NULL THEN
    ...
    INSERT INTO public.access_logs (user_id, patient_id, action, resource, details)
    VALUES (auth.uid(), _patient_id, 'reconcile_state', 'profile', jsonb_build_object('from', v_current_state, 'to', v_new_state));
  END IF;
