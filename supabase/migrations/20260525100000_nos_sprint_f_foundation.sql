-- ============================================================
-- SPRINT F — NUTRITION OPERATING SYSTEM (NOS) — FUNDAÇÃO
-- FitJourney 2.0 — Schema Soberano de Alimentos
-- ============================================================
-- Decisões confirmadas:
--   A. TACO (primário) + USDA (extensão) + brand + custom
--   B. source_priority: TACO > USDA > custom > brand
--   C. Snapshots imutáveis — receitas versionadas
--   D. Identidade canônica com deduplicação por hash nutricional
-- ============================================================

-- ────────────────────────────────────────────────────────────
-- TABELA 1: nos_foods — Fonte unificada de alimentos
-- ────────────────────────────────────────────────────────────
CREATE TABLE IF NOT EXISTS public.nos_foods (
  id                uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  tenant_id         uuid,            -- NULL = alimento global (TACO/USDA)
  nutritionist_id   uuid,            -- NULL = alimento global; preenchido para custom/brand
  
  -- Identidade
  source            text NOT NULL CHECK (source IN ('TACO','USDA','IBGE','custom','brand','supplement')),
  source_id         text,            -- ID original na fonte (ex: TACO código 001)
  source_priority   int NOT NULL DEFAULT 99,
  -- 1=TACO, 2=USDA, 3=IBGE, 4=custom, 5=brand, 6=supplement
  
  name              text NOT NULL,
  name_normalized   text,            -- lowercase sem acentos para deduplicação
  brand             text,
  category          text,
  subcategory       text,
  barcode           text,            -- EAN para marcas
  
  -- Macros SEMPRE por 100g (padrão interno canônico)
  ...
  -- Porção de referência (para exibição)
  portion_g         numeric(8,2) DEFAULT 100,
  portion_label     text,            -- "1 fatia (30g)", "1 colher de sopa (15g)"
  
  -- Identidade canônica para deduplicação (hash dos macros normalizados)
  -- Evita "Arroz branco TACO" e "White rice USDA" virarem entradas duplicadas
  canonical_hash    text,
  canonical_food_id uuid REFERENCES public.nos_foods(id), -- aponta para o canônico se for duplicata
  is_canonical      boolean DEFAULT true,
  -- Metadados
  image_url         text,
  tags              text[],
  is_active         boolean DEFAULT true,
  verified          boolean DEFAULT false,  -- auditado manualmente
...
CREATE INDEX IF NOT EXISTS nos_foods_barcode_idx
  ON public.nos_foods (barcode) WHERE barcode IS NOT NULL;

-- RLS: alimento global (tenant_id NULL) visível para todos
-- alimento custom/brand visível apenas para o tenant dono
...
-- ────────────────────────────────────────────────────────────
-- TABELA 2: nos_recipes — Receitas versionadas do nutricionista
-- ────────────────────────────────────────────────────────────
CREATE TABLE IF NOT EXISTS public.nos_recipes (
  ...
  previous_version_id uuid REFERENCES public.nos_recipes(id), -- cadeia de versões
  ...
  -- Macros POR PORÇÃO (pré-calculados no momento de salvar — NUNCA recalcular runtime)
  ...
  -- Ingredientes: snapshot congelado (IDs + valores no momento de salvar)
  -- Estrutura: [{ food_id, food_name, source, qty_g, kcal, protein, carbs, fat }]
...
-- ────────────────────────────────────────────────────────────
-- TABELA 3: nos_meal_combos — Marmitas, refeições prontas, combos
-- ────────────────────────────────────────────────────────────
CREATE TABLE IF NOT EXISTS public.nos_meal_combos (
  ...
  meal_slot         text,  -- breakfast | lunch | dinner | snack | any
  
  -- Macros totais pré-calculados (congelados no momento de salvar)
  ...
  -- Itens congelados: [{ type, id, name, qty_g, quantity_display, kcal, protein, carbs, fat, image_url }]
  -- type = 'food' | 'recipe'
  items             jsonb NOT NULL DEFAULT '[]',
  
  -- Substituições pré-definidas (já no formato soberano)
...
CREATE INDEX IF NOT EXISTS nos_meal_combos_slot_idx
  ON public.nos_meal_combos (meal_slot, is_active);
...
-- ────────────────────────────────────────────────────────────
-- TABELA 4: nos_nutritionist_library — Biblioteca pessoal
-- ────────────────────────────────────────────────────────────
CREATE TABLE IF NOT EXISTS public.nos_nutritionist_library (
  ...
  -- Snapshot completo no momento de adicionar à biblioteca
  -- Assim a biblioteca funciona offline e sem joins
...
-- ────────────────────────────────────────────────────────────
-- FUNÇÃO: nos_canonical_hash — gera hash de identidade canônica
-- Deduplicação: alimentos com mesmo nome normalizado + macros
-- similares (±2%) são candidatos ao mesmo canônico
-- ────────────────────────────────────────────────────────────
...
-- ────────────────────────────────────────────────────────────
-- FUNÇÃO: nos_search_foods — busca full-text com prioridade
-- Retorna até 20 resultados ordenados por source_priority
-- ────────────────────────────────────────────────────────────
...
-- ────────────────────────────────────────────────────────────
-- SEED: 60 alimentos TACO representativos
-- Cobre os grupos principais usados em clínica BR
-- source_priority = 1 (TACO é base primária)
-- Todos os valores são por 100g, conforme TACO 2011
-- ────────────────────────────────────────────────────────────
...
-- Atualizar hash canônico nos seeds
UPDATE public.nos_foods
SET canonical_hash = public.nos_canonical_hash(name, kcal_100g, protein_100g, carbs_100g, fat_100g),
    name_normalized = lower(regexp_replace(unaccent(name), '[^a-zA-Z0-9 ]', '', 'g'))
WHERE canonical_hash IS NULL AND source IN ('TACO','supplement');

RAISE NOTICE 'NOS Sprint F: schema criado, % alimentos inseridos.',
  (SELECT COUNT(*) FROM public.nos_foods);
