...
BEGIN
  ...
  IF _item_count = 0 THEN
    ...
    VALUES ('EMPTY_PLAN_PUBLISH_BLOCKED', 'Publicação bloqueada: plano sem refeições', jsonb_build_object('plan_id', _plan_id, 'nutritionist_id', _nutritionist_id));
    RAISE EXCEPTION 'EMPTY_PLAN: Não é possível publicar um plano sem refeições.';
  END IF;
