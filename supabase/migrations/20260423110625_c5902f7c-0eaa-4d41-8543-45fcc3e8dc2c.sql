-- 1. Coluna para marcar planos incompletos (não bloqueia salvamento)
ALTER TABLE public.meal_plans
  ADD COLUMN IF NOT EXISTS totals_status TEXT NOT NULL DEFAULT 'ok';

COMMENT ON COLUMN public.meal_plans.totals_status IS 
  'ok | incomplete — marca planos cujos totais não foram calculados (não bloqueia uso)';

-- 2. RPC pública para recálculo sob demanda
...
EXCEPTION WHEN OTHERS THEN
  -- Fallback: nunca lança erro — apenas marca como incompleto para correção async
  UPDATE public.meal_plans
...
GRANT EXECUTE ON FUNCTION public.calculate_plan_totals(UUID) TO authenticated, anon, service_role;

-- 3. Atualiza trigger para também sincronizar totals_status
...
-- 4. Sincronização única para planos existentes
UPDATE public.meal_plans mp
SET totals_status = CASE
  WHEN COALESCE(mp.total_calories, 0) = 0 THEN 'incomplete'
  ELSE 'ok'
END;