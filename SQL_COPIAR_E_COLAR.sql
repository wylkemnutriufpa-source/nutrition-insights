...
WHERE t.created_at >= '2026-05-21'  -- Templates criados hoje
  AND t.category IN ('saude', 'emagrecimento', 'hipertrofia', 'clinico')
  AND t.name LIKE '%kcal'  -- Templates têm kcal no nome
  AND jsonb_array_length(t.meals) > 0  -- Só templates com meals
...
-- Inserir templates de restrições
INSERT INTO v3_diet_templates (
  slug,
  title,
  description,
...
WHERE t.dietary_restrictions IS NOT NULL
  AND t.dietary_restrictions::text != 'null'
  AND t.dietary_restrictions::text != '""'
  AND t.dietary_restrictions::text != ''
  AND jsonb_array_length(t.meals) > 0  -- Só templates com meals
ON CONFLICT (slug) DO UPDATE SET
  title = EXCLUDED.title,
